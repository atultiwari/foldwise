...
**Phases 0–3 complete.** The data pipeline, the biophysics core and the folding
trajectory engine are built and validated. There is no application yet — the
renderer is next.
...
| 4 · Renderer | three.js cartoon, instanced atoms, surface | ⏳ **Next** |
...
pnpm test       # core maths + trajectory engine          -> 131 passed
...
`apps/web`, `packages/render` and `packages/ui` are created when their phase
begins, rather than sitting empty.
